...
export default function SuperAdmin() {
  ...
  // ⚠️ REPLACE THIS WITH YOUR EMAIL SO NOBODY ELSE CAN ACCESS THIS PAGE!
  ...
  const [activeTab, setActiveTab] = useState('doctors') // 'doctors' or 'payments'
  ...
  const init = async () => {
    ...
    const { data: docs } = await supabase
      .from('listings')
      .select('*, pets(owner_name, avatar_url)')
      .eq('is_service', true)
      .eq('brand', 'Doctor')
      .order('created_at', { ascending: false })
    
    // 2. Fetch All Appointments globally
    const { data: appts } = await supabase
      .from('appointments')
      .select('*, listings(name, price), pets(owner_name)')
      .order('created_at', { ascending: false })
  ...
  // Admin function to verify a payment manually after checking their bank app
  const verifyPayment = async (id) => {
    ...
    if (!error) {
       ...
       alert('Payment marked as mathematically Verified!')
    }
  }
      ...
      <div style={{ maxWidth: 1100, margin: '80px auto', padding: 20 }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1E1347, #6C4BF6)', borderRadius: 16, padding: '30px', color: '#fff', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 10px 30px rgba(108, 75, 246, 0.2)' }}>
           ...
           <div style={{ display: 'flex', gap: 20, textAlign: 'center' }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.1)', padding: '10px 20px', borderRadius: 12 }}>
                ...
                <div style={{ fontSize: '0.75rem', textTransform: 'uppercase', opacity: 0.8, letterSpacing: 1 }}>Bookings</div>
              </div>
           </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
          <button onClick={() => setActiveTab('doctors')}
            style={{ flex: 1, padding: '14px', border: 'none', borderRadius: 12, background: activeTab === 'doctors' ? '#FF6B35' : '#fff', color: activeTab === 'doctors' ? '#fff' : '#6B7280', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '1rem', cursor: 'pointer', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
            🏥 Manage Doctors
          </button>
          <button onClick={() => setActiveTab('payments')}
            style={{ flex: 1, padding: '14px', border: 'none', borderRadius: 12, background: activeTab === 'payments' ? '#FF6B35' : '#fff', color: activeTab === 'payments' ? '#fff' : '#6B7280', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '1rem', cursor: 'pointer', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
            💰 Payments & UTRs {appointments.filter(a => a.payment_status === 'paid').length > 0 && `(Verify ${appointments.filter(a => a.payment_status === 'paid').length})`}
          </button>
        </div>
             ...
             {doctors.map(doc => (
               <div key={doc.id} className="card" style={{ padding: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                     <img src={doc.image_url || 'https://via.placeholder.com/150'} style={{ width: 50, height: 50, borderRadius: '50%', objectFit: 'cover' }} />
                     <div>
                       <div style={{ fontWeight: 800, color: '#1E1347', fontSize: '1.1rem' }}>{doc.name}</div>
                       <div style={{ fontSize: '0.8rem', color: '#6B7280' }}>{doc.meant_for || 'Clinic'}</div>
                     </div>
                  </div>
                  <hr style={{ border: 'none', borderTop: '1px solid #EDE8FF', margin: '10px 0' }}/>
                  <div style={{ fontSize: '0.85rem', color: '#374151', marginBottom: 4 }}>📍 {[doc.city, doc.state].join(', ')}</div>
                  ...
                  {/* Security verification to show they own it */}
                   ...
                   {appointments.map(appt => (
                     <tr key={appt.id} style={{ borderBottom: '1px solid #EDE8FF', fontSize: '0.9rem', color: '#1E1347', fontWeight: 700 }}>
                        ...
                        <td>
                          ...
                          {/* CRITICAL: The user has submitted a UTR, Admin needs to verify it */}
