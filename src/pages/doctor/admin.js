...
// Automatically generate 30-min time slots from 8:00 AM to 8:00 PM for doctors to pick from
const TIME_SLOTS = []
for (let h = 8; h <= 20; h++) {
...
export default function DoctorDashboard() {
  ...
  // Schedule state: simple object grouping arrays of selected time slots per day
  ...
  const init = async () => {
    ...
    // Check if this user already has a Doctor profile (stored as a 'services' listing)
    ...
    if (listingData) {
      ...
      setForm({
        ...
        clinic_name: listingData.meant_for || '', 
        ...
        existing_image_url: listingData.image_url || null,
        images: [], imagePreviews: []
      })
      if (listingData.schedule) {
        setSchedule(listingData.schedule)
      }

      // Load appointments for this clinic
      const { data: appts } = await supabase
        .from('appointments')
        .select('*, pets(pet_name, owner_name, avatar_url, emoji)')
  ...
  // Toggle time slot in a specific day
  ...
  const handleImageSelect = (e) => {
    ...
    setForm(prev => ({
      ...prev,
      images: [file], // Replace existing selection with the new one
      imagePreviews: [URL.createObjectURL(file)]
    }))
  }

  const handleSaveProfile = async () => {
    if (!form.name.trim() || !form.city.trim() || !form.price) {
      alert('Please fill out all required fields (Name, City, Consultation Fee)')
      return
    }
    ...
    try {
      ...
      const payload = {
        ...
        category: 'services',
        is_service: true,
        brand: 'Doctor', // We use brand to uniquely identify Doctors in our listings
        is_active: true,
        name: form.name.trim(),
        meant_for: form.clinic_name.trim() || 'Independent Doctor', // Re-using meant_for for clinic name
      ...
      if (listingId) {
        ...
        showToast('🎉 Doctor Profile Created! You are now live on Pawverse.')
      }
  ...
  const updateAppointmentStatus = async (appt, status) => {
    try {
      ...
      // ⚠️ TRIGGER THE EMAILS!
      await fetch('/api/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          trigger: status === 'confirmed' ? 'APPROVED' : 'REJECTED',
          adminEmail: 'ankur16satya@gmail.com', // Replace with your Admin email
          doctorEmail: user.email, // The doctor's own email from their session
          clientEmail: 'client_placeholder@gmail.com', // In a production app, fetch client email from a public profiles table
          appointmentDetails: {
            appointmentId: appt.id,
            doctorName: form.name,
            clientName: appt.pets?.owner_name,
            date: appt.date,
            time: appt.time_slot,
            fees: form.price
          }
    ...
    <div style={{ background: '#FFFBF7', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1000, margin: '80px auto 40px', padding: 20 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
           <h1 style={{ fontFamily: "'Baloo 2', cursive", color: '#1E1347', fontSize: '2rem', margin: 0 }}>🩺 Doctor Dashboard</h1>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, background: '#fff', padding: 8, borderRadius: 16, boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
          <button onClick={() => setActiveTab('profile')}
            style={{ flex: 1, padding: '12px', border: 'none', borderRadius: 10, background: activeTab === 'profile' ? '#FF6B35' : 'transparent', color: activeTab === 'profile' ? '#fff' : '#6B7280', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '0.9rem', cursor: 'pointer', transition: 'all 0.2s' }}>
            👤 Profile & Schedule
          </button>
          <button onClick={() => setActiveTab('appointments')}
            style={{ flex: 1, padding: '12px', border: 'none', borderRadius: 10, background: activeTab === 'appointments' ? '#FF6B35' : 'transparent', color: activeTab === 'appointments' ? '#fff' : '#6B7280', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '0.9rem', cursor: 'pointer', transition: 'all 0.2s' }}>
            📅 Booking Requests {appointments.filter(a => a.status === 'pending').length > 0 && `(${appointments.filter(a => a.status === 'pending').length})`}
          </button>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            ...
            <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
                ...
                <div style={{ width: 100, flexShrink: 0 }}>
                  <div style={{ width: 100, height: 100, borderRadius: '50%', background: '#F3F0FF', border: '2px dashed #6C4BF6', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', cursor: 'pointer', position: 'relative' }}
                    onClick={() => fileInputRef.current?.click()}>
                    {form.imagePreviews.length > 0 ? (
                      <img src={form.imagePreviews[0]} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : form.existing_image_url ? (
                      <img src={form.existing_image_url} alt="existing" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <span style={{ fontSize: '1.5rem' }}>📸</span>
                    )}
                  </div>
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageSelect} style={{ display: 'none' }} />
                  <div style={{ textAlign: 'center', fontSize: '0.7rem', color: '#6B7280', marginTop: 8, fontWeight: 700 }}>Upload Photo</div>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                 <div>
                    ...
                    <input className="input" value={form.qualifications} onChange={e => setForm(p => ({ ...p, qualifications: e.target.value }))} placeholder="e.g. BVSc & AH" style={{ margin: 0 }} />
                 </div>
                 <div>
                    <label className="label">Experience (Years)</label>
                    <input className="input" type="number" value={form.experience_years} onChange={e => setForm(p => ({ ...p, experience_years: e.target.value }))} placeholder="e.g. 5" min="0" style={{ margin: 0 }} />
                 </div>
              </div>
            ...
            <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                 ...
                 <div style={{ fontSize: '0.75rem', color: '#6B7280', fontWeight: 700 }}>Select free times slots</div>
              </div>
                        ...
                        return (
                          <button key={slot} onClick={() => toggleSlot(day, slot)}
                            style={{ 
                              ...
                              borderColor: isSelected ? '#6C4BF6' : '#E5E7EB',
                              borderRadius: 8, background: isSelected ? '#F3F0FF' : '#fff',
                              color: isSelected ? '#6C4BF6' : '#6B7280',
              ...
              <button onClick={handleSaveProfile} disabled={submitting}
                style={{ width: '100%', padding: '14px', border: 'none', borderRadius: 12, background: 'linear-gradient(135deg,#FF6B35,#6C4BF6)', color: '#fff', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '1rem', cursor: 'pointer', marginTop: 20, opacity: submitting ? 0.7 : 1 }}>
          ...
          <div className="card" style={{ padding: 24, minHeight: 400 }}>
             ...
             {appointments.length === 0 ? (
               <div style={{ textAlign: 'center', padding: '40px 0', color: '#9CA3AF' }}>
                 ...
                 <h3 style={{ fontFamily: "Nunito, sans-serif", fontWeight: 800 }}>No bookings yet</h3>
                 <p style={{ fontSize: '0.85rem' }}>When clients book a slot, they will appear here!</p>
               </div>
             ) : (
                   ...
                   <div key={appt.id} style={{ display: 'flex', alignItems: 'center', padding: 16, border: '1px solid #EDE8FF', borderRadius: 12, background: appt.status === 'pending' ? '#FFFBF7' : '#fff' }}>
                     ...
                     <div style={{ display: 'flex', alignItems: 'center', gap: 12, width: 200 }}>
                       <div style={{ width: 44, height: 44, borderRadius: '50%', background: '#F3F0FF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', overflow: 'hidden' }}>
                         {appt.pets?.avatar_url ? <img src={appt.pets.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : appt.pets?.emoji || '🐾'}
                       </div>
                       <div>
                     ...
                     <div style={{ flex: 1, padding: '0 20px', borderLeft: '1px solid #EDE8FF', borderRight: '1px solid #EDE8FF' }}>
                        <div style={{ display: 'flex', gap: 16 }}>
                          <div>
                            <div style={{ fontSize: '0.7rem', color: '#9CA3AF', fontWeight: 800 }}>DATE</div>
                            <div style={{ fontSize: '0.9rem', color: '#FF6B35', fontWeight: 800 }}>{new Date(appt.date).toLocaleDateString()}</div>
                          </div>
                          <div>
                            <div style={{ fontSize: '0.7rem', color: '#9CA3AF', fontWeight: 800 }}>TIME SLOT</div>
                            <div style={{ fontSize: '0.9rem', color: '#6C4BF6', fontWeight: 800 }}>{appt.time_slot}</div>
                          </div>
                        </div>
                     </div>
                     ...
                     <div style={{ width: 180, textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                       ...
                       {appt.status === 'confirmed' && (
                         <span style={{ padding: '4px 10px', borderRadius: 20, background: '#E8F8E8', color: '#22C55E', fontWeight: 800, fontSize: '0.75rem' }}>✅ Confirmed</span>
                       )}
                       {appt.status === 'rejected' && (
                         <span style={{ padding: '4px 10px', borderRadius: 20, background: '#FFE8E8', color: '#FF4757', fontWeight: 800, fontSize: '0.75rem' }}>❌ Rejected</span>
                       )}
                     </div>
